...
if diffusers.__version__ < "0.36.0":
    from diffusers.models.vae import Decoder, DecoderOutput
else:
    from diffusers.models.autoencoders.vae import Decoder, DecoderOutput
from torch.nn import Conv2d, Dropout, Mish, SiLU, GELU, ReLU, GroupNorm
...
def sync_tiled_gn_processor(self):  # GroupNorm
    def forward(tiles):
        ...
        for tile in tmp_tiles:
            mean = mean + tile.mean(-1, keepdim=True) * float(
                tile.shape[-1] / num_elements
            )
        ...
        for shape, tile in zip(shapes, tmp_tiles):
            h, w = shape
            tile = rearrange(
                (tile - mean) / (var + self.eps).sqrt(),
                "b g (c h w) -> b (g c) h w",
                h=h,
                w=w,
            )
            tiles.append(
                tile * self.weight.unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(tiles, temb):
        ...
        if self.upsample is not None:
            if hidden_states[0].shape[0] >= 64:
                ...
                hidden_states = [
                    hidden_state.contiguous() for hidden_state in hidden_states
                ]
        ...
        if temb is not None and self.time_embedding_norm == "scale_shift":
            ...
            hidden_states = [
                hidden_state * (1 + scale) + shift for hidden_state in hidden_states
            ]
        ...
        output_tensor = [
            (tile + hidden_state) / self.output_scale_factor
            for (tile, hidden_state) in zip(tiles, hidden_states)
        ]
...
def tiled_upsample2d_processor(self):
    def forward(hidden_states, output_size=None):
        ...
        if dtype == torch.bfloat16:
            hidden_states = [
                hidden_state.to(torch.float32) for hidden_state in hidden_states
            ]
        ...
        if output_size is None:
            hidden_states = [
                F.interpolate(hidden_state, scale_factor=2.0, mode="nearest")
                for hidden_state in hidden_states
            ]
        else:
            hidden_states = [
                F.interpolate(hidden_state, size=output_size, mode="nearest")
                for hidden_state in hidden_states
            ]
...
def sync_tiled_decode_processor(self, max_tiles_per_batch=8):
    def sync_tiled_decode(
        z: torch.FloatTensor, return_dict: bool = True
    ) -> Union[DecoderOutput, torch.FloatTensor]:
        r"""
        Decode a batch of images using a synchronized tiled decoder.

        When the number of tiles exceeds max_tiles_per_batch, tiles are
        decoded in smaller chunks to avoid OOM at ultra-high resolutions
        (e.g. 4096x4096 = 121 tiles). Sync GroupNorm is computed within
        each chunk rather than globally, which may introduce minor
        inconsistencies between chunks but avoids GPU memory exhaustion.
        ...
        # If tile count is within budget, decode all at once (full sync GroupNorm).
        # Otherwise, decode in chunks to prevent OOM.
        num_tiles = len(tiles)
        if num_tiles <= max_tiles_per_batch:
            decoded = self.decoder(tiles)
        else:
            decoded = []
            for chunk_start in range(0, num_tiles, max_tiles_per_batch):
                chunk = tiles[chunk_start : chunk_start + max_tiles_per_batch]
                decoded_chunk = self.decoder(chunk)
