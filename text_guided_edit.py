...
from inversion_utils import DDIM_tiled_inversion
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Text-guided high-resolution image editing with EditCrafter."
    )
    parser.add_argument(
    ...
    parser.add_argument("--config", type=str, default="./configs/sd2.1_1024x1024.yaml")
...
def pipeline_processor(
    ...
    def forward(
        ...
        do_classifier_free_guidance = is_cfgpp or (
            guidance_scale > 1.0 and not is_cfgpp
        )
    ...
    def process_with_ndcfg_tau(
        ...
        assert not is_cfgpp, "CFG++ is not supported without classifier-free guidance"
        latents = self.scheduler.step(
            noise_pred_vanilla, t, latents, **extra_step_kwargs, return_dict=False
        )[0]
        return latents

    def cfg_process(latents, t, noise_pred, noise_pred_basis):
        at = alpha(t)
        at_prev = alpha(t - self.skip)
        latents = (latents - (1 - at).sqrt() * noise_pred) / at.sqrt()
        latents = at_prev.sqrt() * latents + (1 - at_prev).sqrt() * noise_pred_basis
        return latents
...
def main():
    ...
    ddim_inv = DDIM_tiled_inversion(
        pipeline, num_inference_steps=config.num_inference_steps
    )
    text_embeddings = ddim_inv.init_prompt(args.inversion_prompt)
    ...
    latents = inv_latent.to(accelerator.device, dtype=weight_dtype)
    ...
    for i in range(num_batches):
        ...
        for n in range(config.num_iters_per_prompt):
            ...
            images = pipeline.forward(
                output_prompts, generator=None, latents=latents
            ).images
                ...
                with open(os.path.join(logging_dir, f"{total_num}.txt"), "w") as f:
                    f.writelines(
                        [
                            ...
                            f"inversion_prompt: {args.inversion_prompt}\n",
